...
namespace Spotify.Lib.Connect
{
    ...
    internal class DealerClient : IDisposable
    {
        ...
        private readonly CoreSocket _webSocket = new();
        ...
        internal static DealerClient Instance { get; private set; }
        ...
        private void WebSocketOnSocketConnected(object sender, string e)
        {
            ...
            Debug.WriteLine("Dealer connected! host: {0}", e);
        }
        ...
        private async void WebSocketOnMessageReceivedAsync(object sender, string e)
        {
            ...
            switch (resolvedType)
            {
                ...
                case MessageType.message:
                    ...
                    {
                        Debug.WriteLine("Failed handling message: " + obj + " " + ex);
                    }
                    ...
                    else
                    {
                        if (uri == "hm://playlist/v2/user/7ucghdgquf6byqusqkliltwc2/rootlist")
                        {
                            ...
                            foreach (var payload in payloadsStr)
                            {
                                ...
                                var revisionasBase64 = PlaylistModificationInfo.Parser.ParseFrom(bs)
        ...
        private async Task SendReply(string key, RequestResult result)
        {
        ...
        public async Task<bool> Connect()
        {
            try
            {
                await _webSocket.ConnectSocketAsync(new Uri(
                        $"wss://{(await ApResolver.GetClosestDealerAsync()).Replace("https://", string.Empty)}/?access_token={(await SpotifyClient.Instance.Tokens.GetToken(CancellationToken.None, "playlist-read")).AccessToken}"))
                    ;
                return true;
            }
            catch (Exception x)
        ...
        internal void AddMessageListener(IMessageListener listener, params string[] uris)
        {
            lock (_msgListeners)
            {
                ...
                _msgListeners.AddOrUpdate(listener, uris.ToList());
        ...
        internal void AddRequestListener(IRequestListener listener, string uri)
        {
            lock (ReqListeners)
            {
                ...
                ReqListeners.AddOrUpdate(uri, listener);
