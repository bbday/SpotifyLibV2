...
namespace SpotifyLibrary.Audio
{
    public class PagesLoader
    {
        ...
        public static PagesLoader From(IMercuryClient mercury, Spotify.Player.Proto.Context context)
        {
        ...
        public List<ContextTrack> FetchTracks(string contextUrl)
        {
            var mercuryResponse =
                AsyncContext.Run(() =>
                    _mercuryClient.SendAsync(new JsonMercuryRequest<string>(RawMercuryRequest.Get(contextUrl))));
            return ProtoUtils.JsonToContextTracks(JObject.Parse(mercuryResponse)["tracks"] as JArray ?? throw new InvalidOperationException());

        }
        public List<ContextTrack> ResolvePage(ContextPage page)
        {
        ...
        public List<ContextTrack> GetPage(int pageIndex)
        {
            ...
            if (pageIndex == 0 && !_pages.Any() && ResolveUrl != null)
            {
                var resolvedPages = AsyncContext.Run(() => 
                    _mercuryClient.SendAsync(MercuryRequests.ResolveContext(ResolveUrl)));
